# Feature Acceptance: {{FEATURE_ID}} {{FEATURE_NAME}}

**Work item**: {{FEATURE_ID}}
**Review status**: {{REVIEW_STATUS}}
**Human decision**: {{PENDING_OR_DECISION}}
**Decided by**: {{PENDING_OR_DECIDER}}
**Decision date**: {{PENDING_OR_DECISION_DATE}}
**Decision evidence**: {{PENDING_OR_DECISION_EVIDENCE}}
**Spec**: {{SPEC_PATH}}
**Implementation revision**: {{IMPLEMENTATION_REVISION}}

## Scenario Evidence

| Scenario | Result | Evidence | Notes |
|---|---|---|---|
| {{SCENARIO_ID}} | {{SCENARIO_RESULT}} | {{SCENARIO_EVIDENCE}} | {{SCENARIO_NOTES}} |

## Applicable Quality Gates

| Gate | Applicability | Result | Evidence / reason |
|---|---|---|---|
| Tests | required | {{TEST_RESULT}} | {{TEST_EVIDENCE}} |
| Coverage policy | {{COVERAGE_APPLICABILITY}} | {{COVERAGE_RESULT}} | {{COVERAGE_EVIDENCE_OR_REASON}} |
| CI | {{CI_APPLICABILITY}} | {{CI_RESULT}} | {{CI_EVIDENCE_OR_REASON}} |
| Code review | {{CODE_REVIEW_APPLICABILITY}} | {{CODE_REVIEW_RESULT}} | {{CODE_REVIEW_EVIDENCE_OR_REASON}} |
| Security review | {{SECURITY_APPLICABILITY}} | {{SECURITY_RESULT}} | {{SECURITY_EVIDENCE_OR_REASON}} |

## Blockers

- {{BLOCKER_OR_NONE}}

## Follow-ups

| Type | ID | Description | Blocking |
|---|---|---|---|
| {{FOLLOW_UP_TYPE}} | {{FOLLOW_UP_ID}} | {{FOLLOW_UP_DESCRIPTION}} | {{FOLLOW_UP_BLOCKING}} |

## Human Decision Notes

Pending human decision.
