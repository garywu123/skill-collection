...
Open `.specify/artifact-index.yaml` in full only when it is clearly small. Stop
...
may instead cite an approved product/roadmap `not applicable` rationale stating
...
3. Draft one L0 navigation map and one L1 global-shell skeleton. Do not draw
   feature screens.
4. Use [the UI structure template](assets/ui-structure-template.md).

For a large roadmap spanning multiple product domains, draft the global shell
and one human-selected domain per round. Record remaining domains as unresolved
